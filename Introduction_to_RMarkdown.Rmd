...
Code chunks don't have to be in R!
...
Note there are chunk options `fig.width` and `fig.height` you can change.
...
Use the `output:` line in the YAML header to change the output format to e.g. `word_document" or "pdf_document". (These may require extra software to be installed.)
...
\
This takes _one line_ of code in RMarkdown. Example based on [this post](https://holtzy.github.io/Pimp-my-rmd/#use_dt_for_tables).
