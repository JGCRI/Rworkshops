...
### Geoms that don't really depend on the data
...
iris_plot + 
  ...
  # a-b line
  geom_abline(size = 10, alpha = 0.25, slope = 0.3)
```
...
color aesthetic to a constant value, and be done. This works, but it's not ideal. Better:
...
![Image name](https://imgs.xkcd.com/comics/convincing.png)
...
**Interestingly** (I just realized this in preparing these slides) this seems to be wrong or at least incomplete: looking at the plots above, there's a fourth highest-level attribute, `title`, which isn't inheriting from `text`. I don't understand why not.
